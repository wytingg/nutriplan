...
from evaluation.evaluation import NutriPlanEvaluator
...
class NutriPlanTrainer:
    ...
    def __init__(self, args):
        ...
        is_large_model = any(name in args.model_name for name in ["Qwen2-7B", "Qwen2", "Mistral", "gemma"])
        ...
        # Phi-3 需要 eager attention 避免缓存错误
        if "Phi-3" in args.model_name or "phi-3" in args.model_name:
            model_kwargs["attn_implementation"] = "eager"
            print("✓ Using eager attention for Phi-3")
        ...
        #禁用梯度检查点（与 FP16 冲突导致 NaN）
        # if hasattr(self.model, 'gradient_checkpointing_enable'):
        #     self.model.gradient_checkpointing_enable()
        #     print("✓ Gradient checkpointing enabled")

        # 不要手动 .to(device)，因为 device_map="auto" 已经自动分配了
