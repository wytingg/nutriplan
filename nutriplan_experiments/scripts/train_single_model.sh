...
if [ -d "${OUTPUT_DIR}/best_model" ]; then
    ...
    echo "================================================================"
    echo "Location: ${OUTPUT_DIR}/best_model"
    echo ""

    # Auto-skip if called from batch script (no interaction)
    if [ -t 0 ]; then
        # Interactive mode - ask user
        echo "Options:"
        echo "  1. Skip training (recommended if training completed successfully)"
        echo "  2. Continue training anyway (will overwrite existing model)"
        echo ""
        read -p "Skip training? (y/n): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            echo "✅ Skipping training for ${MODEL_SHORT}_seed${SEED}"
            echo "   Model location: ${OUTPUT_DIR}/best_model"
            exit 0
        else
            echo "⚠️  Proceeding with training (existing model will be overwritten)"
        fi
    else
        # Non-interactive mode (called from batch script) - auto-skip
        echo "✅ Auto-skipping (non-interactive mode): ${MODEL_SHORT}_seed${SEED}"
        echo "   Model location: ${OUTPUT_DIR}/best_model"
        exit 0
    fi
fi
...
if [ "$IS_LARGE" = true ]; then
    GRAD_ACCUM=1
    BATCH_SIZE=4
    echo "📊 Large model detected - Using gradient_accumulation=1, batch_size=4 (effective=4)"
else
    GRAD_ACCUM=4
    BATCH_SIZE=4
    echo "📊 Small model detected - Using gradient_accumulation=4, batch_size=4 (effective=16)"
fi
