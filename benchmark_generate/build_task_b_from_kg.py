...
# -*- coding: utf-8 -*-
"""
Task B Dataset Generator - Hybrid Approach (KG + External Data) - FIXED VERSION

✅ 关键修复（基于Task A经验）：
1. **单位转换**: RNI是全天需求，但instruction和output都基于一餐（33%全天）
2. **逻辑一致性**: Instruction要求X kcal/餐 → Output生成接近X的食谱
3. **生成后验证**: 检查生成的食谱营养是否在目标范围内（60%-150%）
4. **测试模式**: 支持 --test 快速生成10个样本验证
...
INSTRUCTION_TEMPLATES = [
    {
        "template": "I am a {age}-year-old {gender} with {physiological_state}. Based on my health condition and nutritional needs, please create a recipe for ONE MEAL that meets my requirements. I like {liked_sample} but dislike {disliked_sample}. Please generate a complete recipe with ingredients, quantities, and cooking instructions.",
        "type": "health_condition"
    },
    {
        "template": "Please design a recipe for me for ONE MEAL based on my nutritional requirements: {rni_sample}. I prefer dishes with {liked_sample}, and I cannot eat {disliked_sample}. Make sure the recipe is balanced and meets my meal targets.",
        "type": "nutrition_target"
    },
    {
        "template": "As a {age}-year-old {gender}, I need help creating ONE MEAL. My physiological state is {physiological_state}, which requires specific nutrition management. Generate a recipe using {liked_sample}, avoiding {disliked_sample}, that aligns with my meal nutritional targets.",
        "type": "meal_planning"
    },
    {
        "template": "I'm looking for a recipe for ONE MEAL that incorporates {liked_sample} while excluding {disliked_sample}. My nutritional requirements per meal are: {rni_sample}. Please create a dish that satisfies these constraints and tastes good.",
        "type": "preference_focused"
    },
    {
        "template": "Generate a {physiological_state}-friendly recipe for ONE MEAL for a {gender} aged {age}. The recipe should use ingredients I enjoy ({liked_sample}), avoid what I dislike ({disliked_sample}), and meet my meal nutritional targets.",
        "type": "condition_specific"
    },
    {
        "template": "Based on my nutritional requirements for ONE MEAL ({rni_sample}), create a balanced recipe. I have preferences for {liked_sample} and restrictions against {disliked_sample}. Please ensure the recipe is practical and delicious.",
        "type": "rni_constraint"
    },
    {
        "template": "I need a customized recipe for ONE MEAL as a {age}-year-old {gender} managing {physiological_state}. Please use {liked_sample} in the recipe, avoid {disliked_sample}, and make sure it meets my meal nutritional needs.",
        "type": "customized_request"
    },
    {
        "template": "Create a recipe for ONE MEAL that matches my nutritional profile: {rni_sample}. I want to include {liked_sample} and exclude {disliked_sample}. The recipe should be suitable for someone with {physiological_state}.",
        "type": "profile_matching"
    },
    {
        "template": "As someone with {physiological_state}, I need recipes carefully designed for my condition. I'm a {age}-year-old {gender}. Please create ONE MEAL using {liked_sample}, avoiding {disliked_sample}, that meets my meal nutritional targets.",
        "type": "health_aware"
    },
    {
        "template": "Design a recipe for ONE MEAL that achieves these nutritional targets: {rni_sample}. My ingredient preferences include {liked_sample}, and I need to avoid {disliked_sample}. Make it both nutritious and appetizing.",
...
def select_instruction_template(user_profile, seed=0):
    ...
    # Sample RNI values for template (✅ 转换为一餐目标，33%全天)
    rni = user_profile['nutrition_rni']
    rni_items = []

    # ✅ 关键修复：RNI 是全天，但 instruction 应该说一餐目标（33%全天）
    meal_ratio = 0.33

    if rni.get('energy_kcal'):
        meal_energy = round(rni['energy_kcal'] * meal_ratio)
        rni_items.append(f"{meal_energy} kcal energy")
    if rni.get('protein_g'):
        meal_protein = round(rni['protein_g'] * meal_ratio)
        rni_items.append(f"{meal_protein}g protein")
    if rni.get('fiber_g'):
        meal_fiber = round(rni['fiber_g'] * meal_ratio)
        rni_items.append(f"{meal_fiber}g fiber")
    if rni.get('sodium_mg'):
        meal_sodium = round(rni['sodium_mg'] * meal_ratio)
        rni_items.append(f"{meal_sodium}mg sodium")
...
def select_ingredients_hybrid(user_profile, seed=0):
    ...
    # ✅ 修复：liked食材使用宽松验证，避免被EXCLUDE_KEYWORDS排除
    liked_proteins = [ing for ing in matched_liked if is_protein_source(ing) and is_valid_for_liked(ing)]
    ...
    # STEP 2: Carb
    # ✅ 修复：liked食材使用宽松验证
    liked_carbs = [ing for ing in matched_liked if is_carb_source(ing) and is_valid_for_liked(ing)]
    ...
    if matched_liked:
        ...
        # ✅ 修复：使用宽松验证，优先添加用户明确要求的食材
        added_liked = []
        for liked_ing in remaining_liked:
            if is_valid_for_liked(liked_ing):
                added_liked.append(liked_ing)
                # ✅ 增加上限：最多添加3个（从2改为3）
                if len(added_liked) >= 3:
                    break

        # ✅ 修复：确保至少添加2个（如果有的话），不是随机1-2个
        if added_liked:
            num_to_add = min(len(added_liked), max(2, random.randint(2, 3)))
            for ing in random.sample(added_liked, num_to_add):
                # ✅ 放宽限制：最多7个食材（从5改为7），为liked食材留更多空间
                if len(selected) < 7:
                    selected.append(ing)
...
def optimize_quantities_rni_hybrid(ingredients, nutrition_rni, servings=4, max_iterations=20):
    """
    使用外部RecipeNutritionCalculator进行精确营养计算

    ✅ 关键修复：
    1. RNI是全天需求，但食谱是一餐（33%全天）
    2. 使用绝对值而非百分比进行优化（修复蛋白质超标问题）
    3. 增加迭代次数到10次
    """
    # Extract RNI targets (全天) - 添加None值检查
    target_energy_daily = nutrition_rni.get('energy_kcal') or 2000
    target_protein_daily = nutrition_rni.get('protein_g') or 60
    target_fat_daily = nutrition_rni.get('fat_g') or 65
    target_carb_daily = nutrition_rni.get('carbohydrate_g') or 260

    # ✅ 一餐目标（33%全天，然后除以份数）
    meal_ratio = 0.33
    target_energy_meal = target_energy_daily * meal_ratio
    target_protein_meal = target_protein_daily * meal_ratio
    target_fat_meal = target_fat_daily * meal_ratio
    target_carb_meal = target_carb_daily * meal_ratio

    # Per-serving targets (从一餐总量分配到每份)
    target_energy = target_energy_meal / servings
    target_protein = target_protein_meal / servings
    target_fat = target_fat_meal / servings
    target_carb = target_carb_meal / servings
    ...
    # Iterative optimization (✅ 使用绝对值而非百分比)
    for iteration in range(max_iterations):
        ...
        # ✅ Check convergence (使用绝对值)
        energy_error = abs(actual_energy - target_energy) / target_energy if target_energy > 0 else 0
        protein_error = abs(actual_protein - target_protein) / target_protein if target_protein > 0 else 0
        fat_error = abs(actual_fat - target_fat) / target_fat if target_fat > 0 else 0

        # v6.0: 稍微放宽收敛条件，允许更多样本通过
        if energy_error < 0.15 and protein_error < 0.25 and fat_error < 0.3:
            break

        # ✅ Calculate adjustments (基于绝对值的比率)
        # 能量调整
        if actual_energy > 0:
        ...
        # ✅ v6.0 蛋白质调整（最激进的直接目标法）
        if actual_protein > 0:
            protein_ratio = actual_protein / target_protein
            # 直接计算需要的调整比例
            target_adjustment = 1.0 / protein_ratio
            # v6.0: 最激进的调整范围，快速收敛
            if protein_ratio > 1.2:  # 超标
                protein_adjustment = max(0.30, min(0.85, target_adjustment))  # 从0.35改为0.30
            elif protein_ratio < 0.8:  # 不足
                protein_adjustment = max(1.15, min(2.2, target_adjustment))  # 从2.0改为2.2
            else:  # 接近目标
                protein_adjustment = target_adjustment
        else:
            protein_adjustment = 1.5

        # v4.0 脂肪调整（更激进的直接目标法）
        if actual_fat > 0:
            fat_ratio = actual_fat / target_fat
            target_adjustment = 1.0 / fat_ratio
            if fat_ratio > 1.2:
                fat_adjustment = max(0.6, min(0.9, target_adjustment))  # 从0.7改为0.6
            elif fat_ratio < 0.8:
                fat_adjustment = max(1.1, min(1.5, target_adjustment))  # 从1.3改为1.5
            else:
                fat_adjustment = target_adjustment
        else:
            fat_adjustment = 1.2

        # v4.0 碳水化合物调整（更激进的直接目标法）
        if actual_carb > 0:
            carb_ratio = actual_carb / target_carb
            target_adjustment = 1.0 / carb_ratio
            if carb_ratio > 1.2:
                carb_adjustment = max(0.65, min(0.9, target_adjustment))  # 从0.75改为0.65
            elif carb_ratio < 0.8:
                carb_adjustment = max(1.1, min(1.4, target_adjustment))  # 从1.25改为1.4
            else:
                carb_adjustment = target_adjustment
        else:
            carb_adjustment = 1.15

        # ✅ Apply adjustments (分开处理，避免相互抵消)
        for ing in base_qty:
            if is_protein_source(ing):
                # 蛋白质食材：优先蛋白质调整，能量缩放减弱
                base_qty[ing] *= protein_adjustment
                # 只应用部分能量缩放（避免完全抵消蛋白质调整）
                if energy_scale > 1.0:
                    base_qty[ing] *= min(energy_scale, 1.1)  # 最多增加10%
                elif energy_scale < 1.0:
                    base_qty[ing] *= max(energy_scale, 0.9)  # 最多减少10%
            elif 'oil' in ing or 'butter' in ing:
                # 脂肪食材：优先脂肪调整
                base_qty[ing] *= fat_adjustment
                base_qty[ing] *= (energy_scale ** 0.5)  # 部分能量缩放
            elif is_carb_source(ing):
                # 碳水食材：优先碳水调整
        ...
        for ing in base_qty:
            if ing == 'salt':
                ...
                base_qty[ing] = max(5, min(200, base_qty[ing]))  # v6.0: 从10改为5，进一步降低下限
            elif is_carb_source(ing):
...
def generate_recipe_hybrid(user_profile, seed=0):
    ...
    try:
        ...
        # 计算一餐目标（总量，4份）- 添加None值检查
        target_energy_total = (nutrition_rni.get('energy_kcal') or 2000) * meal_ratio
        actual_energy_total = per_serv['energy_kcal'] * servings

        target_protein_total = (nutrition_rni.get('protein_g') or 60) * meal_ratio
        actual_protein_total = per_serv['protein_g'] * servings

        # 验证逻辑：总量应在目标的 60%-150% 范围内（比Task A宽松，因为是生成任务）
        energy_ratio = actual_energy_total / target_energy_total if target_energy_total > 0 else 1
        protein_ratio = actual_protein_total / target_protein_total if target_protein_total > 0 else 1

        is_valid_nutrition = (0.6 <= energy_ratio <= 1.5) and (0.6 <= protein_ratio <= 1.5)
        ...
        recipe = {
            ...
            'metadata': {
                ...
                'rni_optimization': 'Meal-based (33% daily RNI)',
                'validation': {
                    'is_valid_nutrition': is_valid_nutrition,
                    'energy_ratio': f"{energy_ratio:.2f}",
                    'protein_ratio': f"{protein_ratio:.2f}",
                    'target_energy_meal': f"{target_energy_total:.0f} kcal",
                    'actual_energy_meal': f"{actual_energy_total:.0f} kcal"
                }
            }
        }
...
if __name__ == '__main__':
    ...
    if test_mode:
        print("\n" + "="*80)
        print(f"🧪 TEST MODE: Generating only {test_samples} samples for quick validation")
        print("="*80 + "\n")
    ...
    if test_mode:
        # 测试模式：只使用少量用户
        print(f"\n[5/8] TEST MODE: Using {test_samples} users...")
        TRAIN_SIZE = test_samples
        VAL_SIZE = 0
        TEST_SIZE = 0
        TOTAL_SIZE = test_samples
    else:
    ...
    if test_mode:
        splits = [
            ('train_TEST', train_users, f'{OUTPUT_DIR}/task_b_train_from_kg_TEST.jsonl'),
        ]
    else:
        splits = [
