...
# -*- coding: utf-8 -*-
"""
Test C-Class v2 Generation (Complete & Rigorous Version)
"""

import json
from generate_c_class_full_v3_ENHANCED_FIXED import generate_c_class_sample, parse_recipe_ingredients, recalculate_nutrition

print("="*80)
print("C-Class v2 Dataset Generation Test (3 Samples)")
print("="*80)
    ...
    for v in c_sample['input']['violations']:
        if v['type'] == 'nutrition_violation':
            ...
            if field == 'sodium_mg':
                ...
                print(f"     • Protein: {v['actual']:.1f}g ({v.get('actual_pct', 0):.1f}%) < {v.get('target_pct', 20):.0f}% target ({v['severity']})")
    ...
    for v in c_sample['input']['violations']:
        if v['type'] == 'nutrition_violation':
            ...
            if field == 'sodium_mg':
                ...
                target = targets['energy_kcal_target'] / 4
...
if success_count == len(b_recipes):
    ...
    print(f"  python generate_c_class_full_v2.py")
else:
    print(f"\n⚠ Some samples failed. Please review the output above.")
