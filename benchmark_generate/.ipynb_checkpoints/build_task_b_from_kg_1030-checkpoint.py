...
def optimize_quantities_rni_hybrid(ingredients, nutrition_rni, servings=4, max_iterations=5):
    ...
    ✅ 关键修复：RNI是全天需求，但食谱是一餐（33%全天）
    """
    # Extract RNI targets (全天)
...
def generate_recipe_hybrid(user_profile, seed=0):
    ...
    try:
        ...
        # 计算一餐目标（总量，4份）
        target_energy_total = nutrition_rni.get('energy_kcal', 2000) * meal_ratio
        actual_energy_total = per_serv['energy_kcal'] * servings

        target_protein_total = nutrition_rni.get('protein_g', 60) * meal_ratio
