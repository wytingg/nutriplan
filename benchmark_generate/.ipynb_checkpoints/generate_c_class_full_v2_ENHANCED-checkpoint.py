...
# -*- coding: utf-8 -*-
"""
C-Class Dataset Generator v2 - ENHANCED VERSION (8 Nutrition Violation Types)
...
print("="*80)
print("C-Class Dataset Generator v2 - Enhanced (8 Nutrition Types)")
print("="*80)

# Configuration
NUTRITION_DB = 'top500_nutrition_complete.csv'
...
def inject_protein_low_violation(parsed_ingredients, original_nutrition, targets, servings=4):
    ...
    new_qty = original_qty * random.uniform(0.5, 0.7)
    ...
    if new_nutrition:
        ...
        if energy_per_serv > 0:
            ...
            if protein_pct < target_protein_pct * 0.65:
                violations.append({
                    'type': 'nutrition_violation',
                    'field': 'protein_amdr',
...
def inject_carb_low_violation(parsed_ingredients, original_nutrition, targets, servings=4):
    ...
    new_qty = original_qty * random.uniform(0.4, 0.6)
    ...
    if new_nutrition:
        ...
        if energy_per_serv > 0:
            ...
            if carb_pct < target_carb_pct * 0.6:
                violations.append({
                    'type': 'nutrition_violation',
                    'field': 'carb_amdr',
                    'actual_pct': carb_pct,
...
def inject_saturated_fat_high_violation(parsed_ingredients, original_nutrition, targets, servings=4):
    ...
    new_nutrition = recalculate_nutrition(modified, servings)
    if new_nutrition:
        sat_fat_per_serv = new_nutrition['per_serving']['saturated_fat_g']
        energy_per_serv = new_nutrition['per_serving']['energy_kcal']
...
def inject_sugars_high_violation(parsed_ingredients, original_nutrition, targets, servings=4):
    ...
    new_nutrition = recalculate_nutrition(modified, servings)
    if new_nutrition:
        sugars_per_serv = new_nutrition['per_serving']['sugars_g']
        energy_per_serv = new_nutrition['per_serving']['energy_kcal']
...
def generate_violated_recipe(b_class_recipe):
    ...
    if rand < 0.6:
        # Single nutrition violation (8 types)
