...
class RecipeNutritionCalculator:
    ...
    def parse_ingredient_line(self, ingredient_text):
        ...
        # Pattern 1: [quantity][unit] [ingredient] (no space between qty and unit)
        # Examples: "100g flour", "500ml milk", "2kg chicken"
        pattern1 = r'^([\d\.]+)(g|kg|ml|l|oz|lb|mg)\s+(.+)$'
        match = re.match(pattern1, ingredient_text)
        ...
        # Pattern 2: [quantity] [unit] [ingredient] (space between qty and unit)
        # Examples: "1 cup butter", "2 tablespoons oil"
        pattern2 = r'^([\d\s/\.]+)\s+([\w]+)\s+(.+)$'
        match = re.match(pattern2, ingredient_text)
...
def test_calculator():
    ...
    calc = RecipeNutritionCalculator('work/recipebench/data/raw/foodcom/nutrition_top100_verified.csv')
