...
import json
from generate_c_class_full_v4_FINAL import (
...
print("="*80)
print("C-Class v4 Dataset Generation Test (3 Samples)")
print("="*80)
    ...
    for v in c_sample['input']['violations']:
        if v['type'] == 'nutrition_violation':
            ...
            if field == 'sodium_mg':
                ...
                # 尝试不同的字段名
                sat_fat = corrected_nutrition.get('saturated_fat_g') or \
                          corrected_nutrition.get('saturated_fat', 0)
                sat_fat_kcal = sat_fat * 9
                energy_kcal = corrected_nutrition['energy_kcal']
                if energy_kcal > 0:
                ...
                # 尝试不同的字段名
                sugars = corrected_nutrition.get('sugars_g') or \
                         corrected_nutrition.get('sugars', 0)
                sugars_kcal = sugars * 4
                energy_kcal = corrected_nutrition['energy_kcal']
                if energy_kcal > 0:
...
if success_count == len(b_recipes):
    ...
    print(f"  python generate_c_class_full_v4_FINAL.py")
