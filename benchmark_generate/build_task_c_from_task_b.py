...
v6.1适配（2025-10-30）：
✅ 禁用"删除liked_ingredient"违规类型 - 保护Task B v6.1食材匹配质量
✅ 调整违规类型分布：70%营养 / 15%偏好 / 15%双重
✅ 原因：Task B v6.1已确保liked_ingredients出现在食谱中（70.9%匹配率）
"""
...
def inject_preference_violation(parsed_ingredients, liked_ingredients, disliked_ingredients):
    ...
    ✅ v6.1适配：禁用删除liked_ingredient
    原因：Task B v6.1已经优先保留liked_ingredients (70.9%匹配率)
    删除它们会创建不真实的违规场景
    """
    modified = copy.deepcopy(parsed_ingredients)
    violations = []
    ...
    # ✅ v6.1适配：禁用删除liked_ingredient
    # Task B v6.1已经确保liked_ingredients出现在食谱中
    # 删除它们会破坏v6.1的食材匹配质量
    #
...
def generate_violated_recipe_v2(b_class_recipe):
    ...
    elif rand < 0.85:  # ✅ v6.1: 保持0.85，现在是15%偏好违规（从25%降低）
        # Preference violation only (只添加disliked)
