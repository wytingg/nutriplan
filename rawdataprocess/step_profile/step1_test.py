...
---------------------------------------
python work/recipebench/scripts/step_profile/step1_test.py \
...
def parse_args():
    ...
    ap.add_argument("--buffer-write", type=int, default=2000,
                    help="Write JSONL in batches of N users to reduce I/O overhead")
    ap.add_argument("--no-progress", action="store_true",
                    help="Disable tqdm progress bar to save overhead on TTY")
...
def run(args):
    ...
    # Pre-attach idf/bm25: use category + map to reduce merge overhead
    ev["ingredient"] = ev["ingredient"].astype("category")
    ev["idf"] = ev["ingredient"].map(idf).astype("float32").fillna(1.0)
    ev["idf_bm25"] = ev["ingredient"].map(idf_bm25).astype("float32").fillna(0.0)

    # Ensure stable dtypes to avoid implicit upcast
    ev["AuthorId"] = ev["AuthorId"].astype("int64")
    ev["n_ing"] = ev["n_ing"].astype("int16")
    ev["len_w"] = ev["len_w"].astype("float32")
    ev["time_w"] = ev["time_w"].astype("float32")

    # Fast path: pre-aggregate once
    agg, user_tot = preaggregate_events_fast(
        ev, args.idf_alpha, idf_bm25, args.k1, args.b, avg_n_ing
    )
    # Compute background on aggregated table
    bg_by_ing = (agg["pos_w"] + agg["neg_w"]).groupby(agg["ingredient"]).sum().astype("float64")
    total_bg = float(bg_by_ing.sum())
    p_bg = (bg_by_ing / (total_bg if total_bg > 0 else 1.0)).clip(1e-12, 1.0)
    tot_map = dict(zip(user_tot["AuthorId"].to_numpy(), user_tot[["npos","nneg"]].to_numpy()))

    use_tqdm = (_tqdm is not None) and (not args.no_progress)
    users = agg["AuthorId"].unique().tolist()
    iterator = users if not use_tqdm else _tqdm(users, total=len(users), desc="[step1] users", unit="user", mininterval=1.0)
    ...
    buf, BATCH = [], max(int(args.buffer_write), 1)
