...
def extract_noun_units_from_A(a_table):
    ...
    # —— 读黑名单（读不到就用内置 set）
    try:
        bl = pd.read_csv("work/recipebench/data/config/units_blacklist.csv")
        bl_set = set(bl["token"].astype(str).str.strip().str.lower())
    except Exception:
        bl_set = {"olive","olives","breast","onion powder","tomato paste","tomato sauce"}
    agg = agg[~agg["noun"].isin(bl_set)]
...
# —— 禁止误触发的部位名
PIECE_UNITS.discard('breast')
PIECE_UNITS.discard('wing')
PIECE_UNITS.discard('rib')
...
def load_foodcom(recipes_path, reviews_path=None, enable_four_lanes=True, impute_units=True):
    ...
    # 替换你原来的 UNIT 正则与标准化表（带否定保护 + 允许数字贴连，不用lookbehind）
    NEG_GUARDS = {"egg": r"(?!plant\w*)", "eggs": r"(?!plant\w*)", "lemon": r"(?!grass\w*)", "lemons": r"(?!grass\w*)"}
    def _guard(u):
        g = NEG_GUARDS.get(u, "")
        return f"(?:{re.escape(u)}){g}"

    all_units = list(UNIT_STD_MAP.keys())
    all_units = sorted(all_units, key=len, reverse=True)
    core = "|".join(_guard(u) for u in all_units)

    # 允许前面是行首或非字母（数字可以），后面不是字母
    UNIT_REGEX = re.compile(rf"(?i)(?:^|[^A-Za-z])(?:{core})(?![A-Za-z])")
    ...
    if NOUN_PIECE_WEIGHT:
        # 仅对主通道没识别到标准单位且未被反模式击中的行做名词单位识别
        BLOCK_PATTERNS = [
        ...
        def _blocked_txt(s: str) -> bool:
            s = str(s or "")
            return any(p.search(s) for p in BLOCK_PATTERNS)
        mask_no_std = ingr_df["unit_std"].isna()
        mask_not_blocked = ~ingr_df["ingredient_raw"].map(_blocked_txt)
        mask_no_std = mask_no_std & mask_not_blocked
    ...
    RES_DIR = Path(__file__).resolve().parent / "resources" / "units"

    # --- 1) 加载三表
    def _load_units_tables():
        from pandas.errors import EmptyDataError
        def _read_first(paths, cols):
            for p in paths:
                try:
                    if p.exists():
                        df = pd.read_csv(p)
        ...
        wl = _read_first([CFG_DIR/"units_whitelist.csv", RES_DIR/"units_whitelist.csv"], ["unit","alias"])
        bl = _read_first([CFG_DIR/"units_blacklist.csv", RES_DIR/"units_blacklist.csv"], ["token","reason"])
        cu = _read_first([CFG_DIR/"units_conditional.csv", RES_DIR/"units_conditional.csv"], ["unit","trigger_lemmas","size_key","fdc_id","grams_per_unit","notes"])

        # 归一化（仅当列存在）
        if "unit" in wl.columns:
            wl["unit"] = wl["unit"].astype(str).str.strip().str.lower()
        if "token" in bl.columns:
            bl["token"] = bl["token"].astype(str).str.strip().str.lower()
        if "unit" in cu.columns:
            cu["unit"] = cu["unit"].astype(str).str.strip().str.lower()
        if "trigger_lemmas" in cu.columns:
            cu["trigger_lemmas"] = cu["trigger_lemmas"].astype(str).str.strip().str.lower()
        if "size_key" in cu.columns:
            cu["size_key"] = cu["size_key"].fillna("").astype(str).str.lower()
        return wl, bl, cu

    UNITS_WL, UNITS_BL, COND_UNITS = _load_units_tables()
    UNIT_WL_SET = set(UNITS_WL["unit"].tolist()) if "unit" in UNITS_WL.columns else set()
    UNIT_WL_ALIAS = {}
    if not UNITS_WL.empty and "unit" in UNITS_WL.columns:
        for _, row in UNITS_WL.iterrows():
            unit_val = str(row.get("unit",""))
            alias_val = str(row.get("alias", unit_val)) if "alias" in UNITS_WL.columns else unit_val
            if unit_val:
                UNIT_WL_ALIAS[unit_val] = re.compile(r"\b(?:" + alias_val + r")\b", re.I)
    _bl_list = UNITS_BL["token"].tolist() if (not UNITS_BL.empty and "token" in UNITS_BL.columns) else []
    UNIT_BL_PAT = re.compile(r"(?!x)x") if not _bl_list else re.compile(r"\b(?:" + "|".join(map(re.escape, _bl_list)) + r")\b", re.I)
    ...
    # （2）非捕获版，专供 contains
    PAT_VOLUME_NC = re.compile(rf"\b(?:{QTY})\s*(?:ml|millilit(?:er|re)s?|l|lit(?:er|re)s?|cups?|tsp|teaspoons?|tbsp|tablespoons?|fl\.?\s*oz)\b", re.I)
    PAT_PACK = re.compile(
    ...
    def _conditional_piece_to_grams(row):
        # —— 安全转字符串，避免 float/NaN 触发 split 错误
        def _s(x):
            if x is None:
                return ""
        ...
        ctx = _s(row.get("ctx")) or _s(row.get("ingredient_raw"))
        ctx = ctx.lower()

        if _is_blocked(ctx):
            return (None, None)

        head_raw = _s(row.get("ingredient_head")) or _s(row.get("ingredient_norm"))
        head = head_raw.split(",")[0].split(" ")[0].lower()
    ...
    def parse_units_block(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # contains 用非捕获版，避免捕获组警告
        df["vol_hit"] = texts.str.contains(PAT_VOLUME_NC)
