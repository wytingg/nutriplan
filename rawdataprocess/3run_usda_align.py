...
def load_foodcom(recipes_path, reviews_path=None):
    ...
    # 解析数量/单位（优先使用 qty_raw；否则从 ingredient_raw 抽取）；并进行单位标准化
    qty_list, unit_raw_list, unit_std_list = [], [], []
    for qraw, iraw in zip(ingr_df["qty_raw"], ingr_df["ingredient_raw"]):
        q, u = parse_qty(qraw if qraw is not None and str(qraw).strip() != "" else iraw)
        qty_list.append(q)
        unit_raw_list.append(u)
        unit_std_list.append(normalize_unit_name(u) if u else None)
    ingr_df["qty_parsed"] = qty_list
    ingr_df["unit_raw"] = unit_raw_list
    ingr_df["unit_std"] = unit_std_list
...
# ------------------------ 聚合营养（全营养） ------------------------
def aggregate_recipe_nutrition(ingr_df, mapping_df, fn, nutr, out_dir, default_g=40.0, write_wide=False, wide_max_nutrients=512, agg_batch_size=200000, household_df=None):
    m = mapping_df[["ingredient_norm","fdc_id","fdc_desc","fuzz","score"]].drop_duplicates()
    x = ingr_df.merge(m, on="ingredient_norm", how="left")
    # 确保 fdc_id 可用于与 A 表连接
    try:
        x["fdc_id"] = pd.to_numeric(x["fdc_id"], errors="coerce").astype("Int64")
    except Exception:
        pass
    # 若提供 household A 表且存在数量/单位与 fdc_id，则优先用 A 表计算克重
    grams_by_household = None
    if household_df is not None and not household_df.empty:
        try:
            ...
            hh["grams_per_unit"] = pd.to_numeric(hh["grams_per_unit"], errors="coerce")
            xx = x.merge(hh, left_on=["fdc_id","unit_std"], right_on=["fdc_id","unit"], how="left")
            grams_by_household = (xx["qty_parsed"].astype(float) * xx["grams_per_unit"].astype(float))
            x["grams_household"] = grams_by_household
        except Exception:
            x["grams_household"] = np.nan
    else:
        x["grams_household"] = np.nan
    # 选择克重：优先 household 计算，其次启发式估计，最后默认值
    x["grams_pref"] = x["grams_household"].where(x["grams_household"].notna(), x["grams"]) 
    x["grams_eff"] = x["grams_pref"].where(x["grams_pref"].notna(), default_g)
    # 统计来源计数并写入日志
    try:
        src_household = int(x["grams_household"].notna().sum())
        src_heuristic = int(((x["grams_household"].isna()) & (x["grams"].notna())).sum())
        src_default = int(((x["grams_household"].isna()) & (x["grams"].isna())).sum())
    except Exception:
        src_household = src_heuristic = src_default = 0
    ...
    grams_stats = {
        "n_rows_with_household_grams": src_household,
        "n_rows_with_heuristic_grams": src_heuristic,
        "n_rows_with_default_grams": src_default,
    }
    return wide, core, x, mx, grams_stats
...
def load_usda_and_fdnn(usda_dir, fdnn_dir, filter_types=None):
    ...
    """
    # 以精简列和 dtype 方式加载，降低内存；同时保留可用的扩展列
    ...
    nutr_usda = pd.read_csv(
        ...
        dtype={"id":"int64","name":"string","unit_name":"string","nutrient_nbr":"string","rank":"float32"}
    )
    fn_usda   = pd.read_csv(
        os.path.join(usda_dir, "food_nutrient.csv"), usecols=fn_cols,
        dtype={"id":"Int64","fdc_id":"int64","nutrient_id":"int64","amount":"float32","data_points":"Int64","derivation_id":"Int64"},
        low_memory=False
    )
    ...
    assert {"fdc_id","description","data_type"}.issubset(set(food_fdnn.columns)), "FDNN food.csv 缺少必要列"
    assert {"id","name","unit_name"}.issubset(set(nutr_fdnn.columns)), "FDNN nutrient.csv 缺少必要列"
    assert {"fdc_id","nutrient_id","amount"}.issubset(set(fn_fdnn.columns)), "FDNN food_nutrient.csv 缺少必要列"
...
def main(args):
    ...
    food, nutr, fn, inv_index, nutr_usda_raw, nutr_fdnn_raw = load_usda_and_fdnn(args.usda_dir, args.fdnn_dir, args.filter_usda_types)
    # 导出营养素目录（合并后的 USDA+FNDDS 全部营养类别）
    try:
        nutr_catalog = nutr[["id","name","unit_name","nutrient_nbr","rank"]].drop_duplicates().sort_values(["name","unit_name"]).reset_index(drop=True)
        nutr_catalog.to_csv(os.path.join(args.out_dir, "nutrient_catalog_usda_fndds.csv"), index=False)
    except Exception as e:
        warnings.warn(f"导出 nutrient_catalog_usda_fndds.csv 失败: {e}")
    # 分别导出 USDA 与 FNDDS 的营养素目录
    try:
        usda_catalog = nutr_usda_raw[["id","name","unit_name","nutrient_nbr","rank"]].drop_duplicates().sort_values(["name","unit_name"]).reset_index(drop=True)
        usda_catalog.to_csv(os.path.join(args.out_dir, "nutrient_catalog_usda.csv"), index=False)
    except Exception as e:
        warnings.warn(f"导出 nutrient_catalog_usda.csv 失败: {e}")
    try:
        fdnn_catalog = nutr_fdnn_raw[["id","name","unit_name","nutrient_nbr","rank"]].drop_duplicates().sort_values(["name","unit_name"]).reset_index(drop=True)
        fdnn_catalog.to_csv(os.path.join(args.out_dir, "nutrient_catalog_fdnn.csv"), index=False)
    except Exception as e:
        warnings.warn(f"导出 nutrient_catalog_fndds.csv 失败: {e}")
    ...
    # 加载 household A 表（可选）
    household_df = None
    if getattr(args, "household_weights", None):
        try:
            if os.path.exists(args.household_weights):
                household_df = pd.read_csv(args.household_weights)
        except Exception:
            household_df = None
    wide, core, exploded_ing, matched_ing, grams_stats = aggregate_recipe_nutrition(
    ...
    # 5.x) 实际出现在聚合结果中的营养素清单（长表维度）
    try:
        long_path = os.path.join(args.out_dir, "recipe_nutrients_long.parquet")
        if os.path.exists(long_path):
            rn_long = pd.read_parquet(long_path, columns=["nutrient_name","unit"]).dropna()
            present_nutr = rn_long.drop_duplicates().sort_values(["nutrient_name","unit"]).reset_index(drop=True)
            present_nutr.to_csv(os.path.join(args.out_dir, "present_nutrients_in_recipes.csv"), index=False)
    except Exception as e:
        warnings.warn(f"导出 present_nutrients_in_recipes.csv 失败: {e}")

    # 5.1) 统计：包含“缺克重用 default_grams”的食谱数（至少一个匹配食材缺克重）
    try:
        default_used_recipes = int(
            exploded_ing[(exploded_ing["fdc_id"].notna()) & (exploded_ing["grams"].isna())]["recipe_id"].nunique()
        )
    except Exception:
        default_used_recipes = 0
    ...
    stats = {
        ...
        "n_recipes_using_default_grams": default_used_recipes,
        "grams_source_rows": grams_stats,
    ...
    print(f"   Recipes using default_grams (>=1 matched ingredient missing grams): {default_used_recipes}")
    try:
        print(f"   Household grams rows: {stats['grams_source_rows']['n_rows_with_household_grams']}")
        print(f"   Heuristic grams rows: {stats['grams_source_rows']['n_rows_with_heuristic_grams']}")
        print(f"   Default grams rows: {stats['grams_source_rows']['n_rows_with_default_grams']}")
    except Exception:
        pass
    print("   - ingredient_mapping.parquet")
    print("   - unmatched_ingredients.csv")
    print("   - nutrient_catalog_usda.csv")
    print("   - nutrient_catalog_fdnn.csv")
    print("   - nutrient_catalog_usda_fndds.csv")
    print("   - recipe_nutrients_wide.parquet")
    print("   - recipe_nutrients_long.parquet")
    print("   - present_nutrients_in_recipes.csv")
...
if __name__ == "__main__":
    ...
    p.add_argument("--fdnn_dir", required=True, help="FDNN CSV 目录（food.csv / nutrient.csv / food_nutrient.csv）")
    ...
    p.add_argument("--household_weights", default=None, help="可选：build_household.py 生成的 household_weights_A.csv 路径")
    ARGS = p.parse_args()
    main(ARGS)
